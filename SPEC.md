...
| source | VARCHAR(50) | user_edit, import_facebook, import_instagram, import_twitter, import_google, import_vcard, import_sniffies, import_snapchat, import_csv, merge |
...
### import_jobs
Tracks each import attempt — both file-based uploads and Chrome extension pushes.

| Column | Type | Notes |
|--------|------|-------|
| id | INT AUTO_INCREMENT PK | |
| user_id | INT FK → users | Who triggered this import |
| source_platform | VARCHAR(50) | facebook, instagram, twitter, google_contacts, vcard, csv, sniffies, snapchat |
...
| is_spicy_source | BOOLEAN DEFAULT 0 | Auto-true for sniffies and snapchat |
...
### Data
- **Import** — Access the import tools (file upload + Chrome extension setup). See Data Import System.
- **Export / Backup** — Full data export
- **Default Tags** — Manage system-wide tags
- **Default Groups** — Manage system-wide groups

### Chrome Extension
- **API Token** — Generate/revoke a long-lived API token for use in the Chrome extension. This token is separate from session JWTs and used exclusively by the extension.
- **Allowed Platforms** — Toggle which platforms the extension can submit data from.
...
- vCard (`.vcf`) — standard contact card format, supports multi-contact files
- CSV — with column mapping step

**Chrome extension (browser-based scraping — see Chrome Extension section):**
- Sniffies — profiles, messages, media (always marked spicy)
- Snapchat — chats, profiles, media (always marked spicy)
...
### Import Flow (Chrome Extension)

```
1.  User runs the Chrome extension on Sniffies or Snapchat
2.  Extension scrapes profiles/messages/media in the background
3.  Extension sends normalized payload to POST /api/import/extension
4.  Server creates import_job + import_staging records (status: awaiting_review)
5.  App sends notification + updates progress widget
6.  Same Data Review flow as file imports
```
...
### Automatic Spicy Flagging

Imports from Sniffies and Snapchat automatically set `is_spicy_source = true` on the import job. This causes:
...
All parsers (file-based and extension) must produce records conforming to this format before writing to `import_staging.normalized_data`:
...
`spicy_data` is only present for spicy-source imports. `messages` and `media` may be empty arrays. All fields are optional — parsers include what the platform provides.
...
The Tampermonkey userscripts (`tampermonkey_scripts/`) have been migrated to a proper Chrome extension. The existing scripts (Snapchat CRM Collector v15, Sniffies CRM Collector v9) serve as the baseline — their scraping logic, DOM selectors, and data extraction patterns are preserved and adapted for the MV3 Chrome extension architecture. This allows better media handling (direct API upload instead of browser download folder), more reliable API communication, and a cleaner installation experience.
...
├── background/
│   └── service-worker.js  # Handles API communication, job tracking
├── content-scripts/
│   ├── sniffies.js        # Scraper for sniffies.com
│   └── snapchat.js        # Scraper for web.snapchat.com
├── popup/
│   ├── popup.html         # Extension popup UI
│   ├── popup.js
│   └── popup.css
└── shared/
    ├── api.js             # Kith API client (reusable across scripts)
    ├── normalizer.js      # Platform → normalized format conversion
    └── config.js          # Reads stored config (API URL, token)
```

### Authentication

The extension authenticates using a long-lived API token generated in Kith Settings → Chrome Extension. This token is stored in `chrome.storage.local` and passed as `Authorization: Bearer <token>` on all API calls. It is distinct from session JWTs.

### Popup UI

The extension popup shows:
- Connection status (connected to Kith at `{api_url}`)
- Current page detected (e.g. "Sniffies — ready to collect")
- Start/stop scrape button
- Live progress (profiles found, messages scraped, media queued)
- Last sync status

### What gets scraped (per platform)

**Sniffies:**
- Profile: display name, bio, stats (age, body type, role, HIV status, etc.), profile photos
- All conversations: messages (both directions) + any media shared in chats
- All profiles are spicy-flagged automatically

**Snapchat:**
- Profile: display name, username, Bitmoji if accessible
- All conversations: messages (both directions), any accessible media
- All profiles are spicy-flagged automatically

### Sending to Kith

When scraping is complete, the extension sends a POST to `POST /api/import/extension` with normalized records matching the Normalized Import Format. Media files are uploaded directly to the Kith API (rather than downloaded to the browser's download folder) — the server writes them to the the storage layer mount at `PersonalExports/{Platform}/` and stores the file path reference.

```json
{
  "source_platform": "sniffies",
  "records": [ /* array of normalized import records (see Normalized Import Format) */ ],
  "media_uploads": [ /* base64-encoded media with metadata */ ]
}
```

Media uploads are sent as base64 with metadata:
```json
{
  "filename": "profile_image_001.jpg",
...
The server creates the `import_job` and `import_staging` records, writes media files to `PersonalExports/{Platform}/{contact_name}/`, and responds immediately. The extension shows a "Sent to Kith — ready for review" confirmation in the popup.
...
### Contacts
- `GET /api/contacts` — List (scoped to user unless admin). Supports: ?tag=, ?group=, ?search=, ?sort=, ?sortDir=, ?favorites=
...
### Spicy Profiles
- `GET /api/contacts/:id/spicy` — Get (404 if spicy disabled in settings)
...
### Import — File Upload
- `POST /api/import/upload` — Upload one or more export files. Accepts `multipart/form-data` with fields: `files[]` (multi-file), `source_platform` (facebook | instagram | twitter | google_contacts | vcard | csv). Returns `import_job_id`. Processing is async.
- `POST /api/import/csv` — Legacy CSV import with column mapping (kept for backwards compatibility)

### Import — Chrome Extension
- `POST /api/import/extension` — Receive scraped data from Chrome extension. Body: `{ source_platform, records[], media_uploads[] }`. Auth via long-lived extension token. Returns `import_job_id`.
...
| File uploads | `multer` |
| ZIP extraction (export parsing) | `adm-zip` |
...
```
# Infrastructure (GitLab CI / homelab)
SERVICE=Kith
NAME=Kith
DEPLOYMENT_SLUG=kith
VM=the application server
IMAGE=gitlab.example.com/homelab/knowledgecore/kith
TAG=latest
INTERNAL_PORT=3000
EXTERNAL_PORT=8084
URL=kith.example.com
